...
export default function SiigoClosingWizard() {
    ...
    const renderStep1 = () => (
        <div className="space-y-4 animate-in fade-in slide-in-from-right-4 duration-300">
            <h3 className="text-lg font-bold text-blue-800 border-b pb-2">Paso 1: Apertura de Caja (Siigo)</h3>
            ...
            <div className="flex justify-end pt-4">
                <Button onClick={nextStep} disabled={!formData.branch_id || !formData.shift} className="bg-blue-600 hover:bg-blue-700">
    ...
    const renderStep2 = () => (
        <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
            <h3 className="text-lg font-bold text-blue-800 border-b pb-2">Paso 2: Registro de Productos (Siigo)</h3>
            <p className="text-sm text-gray-500">Agrega los productos vendidos durante el turno.</p>

            {/* Add Product Form */}
            <div className="bg-blue-50 p-4 rounded-xl space-y-4 border border-blue-200">
            ...
            <div className="space-y-2">
                        ...
                        {productsSold.map((item, idx) => (
                            <div key={idx} className="flex items-center justify-between bg-white p-3 rounded-lg border shadow-sm group">
                                ...
                                <div className="flex items-center gap-4">
                                    <span className="font-bold text-blue-700">${formatCurrency(item.quantity * item.price)}</span>
                                    <button
                ...
                {productsSold.length > 0 && (
                    <div className="space-y-4">
                        <div className="flex justify-between items-center bg-blue-100 p-4 rounded-xl border border-blue-200 mt-4">
                            <span className="font-bold text-blue-900">Total Ventas (Productos):</span>
    ...
    const renderStep3 = () => {
        return (
            <div className="space-y-8 animate-in fade-in slide-in-from-right-4 duration-300">
                <h3 className="text-lg font-bold text-blue-800 border-b pb-2">Paso 3: Gastos y Propinas</h3>
    ...
    const renderStep4 = () => {
        ...
        return (
            <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
                <h3 className="text-lg font-bold text-blue-800 border-b pb-2">Paso 4: Arqueo Físico Detallado (Siigo)</h3>
                ...
                <div className="bg-blue-50 p-4 rounded-lg border border-blue-100 mb-2">
                    <p className="font-bold text-blue-800 text-sm flex items-center gap-2">⚠️ Importante</p>
                    <p className="text-xs text-blue-700">El sistema calculará el total automáticamente según las cantidades ingresadas.</p>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <div className="space-y-3">
                        <div className="flex items-center gap-2 mb-2 pb-1 border-b border-blue-200">
                            ...
                            <h4 className="font-bold text-blue-800 header-font">BILLETES</h4>
                        </div>
                    ...
                    <div className="space-y-3">
                        <div className="flex items-center gap-2 mb-2 pb-1 border-b border-blue-200">
                            ...
                            <h4 className="font-bold text-blue-800 header-font">MONEDAS</h4>
                        </div>
                        <div className="grid grid-cols-12 gap-2 text-xs font-bold text-gray-400 uppercase tracking-wide mb-1 text-center">
                            <div className="col-span-4 text-left">Denominación</div>
                            <div className="col-span-3">Cantidad</div>
                ...
                {/* Gran Total */}
                <div className="bg-blue-900 text-white p-4 rounded-xl flex justify-between items-center shadow-lg mt-6">
                    <div>
                ...
                <div className="flex justify-between pt-4">
                    <Button variant="secondary" onClick={prevStep} startIcon={<ArrowLeft className="w-4 h-4" />}>Atrás</Button>
                    <Button onClick={nextStep} disabled={formData.cash_audit_count <= 0} className="bg-blue-600 hover:bg-blue-700">
    ...
    const renderStep5 = () => {
        ...
        return (
            <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
                <h3 className="text-lg font-bold text-blue-800 border-b pb-2">Paso 5: Resumen y Soportes (Siigo)</h3>

                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="space-y-3 bg-gray-50 p-4 rounded-xl">
                        <h4 className="font-display font-bold text-gray-700">Operación (Siigo)</h4>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        ...
                        <div className="bg-white p-3 rounded-lg border border-gray-200">
                            <label className="block text-xs font-bold text-gray-500 mb-2 uppercase tracking-wide">
                                1. Factura Cierre (Siigo) <span className="text-red-500">*</span>
                            </label>
                            {!closingProofUrl ? (
                        ...
                        <div className="bg-white p-3 rounded-lg border border-gray-200">
                            <label className="block text-xs font-bold text-gray-500 mb-2 uppercase tracking-wide">
                                2. Productos Vendidos (Siigo) <span className="text-red-500">*</span>
                            </label>
                            {!productsProofUrl ? (
                                <label className="flex flex-col items-center justify-center w-full h-24 border-2 border-gray-300 border-dashed rounded-lg cursor-pointer hover:bg-gray-50">
                                    <Upload className="w-6 h-6 text-gray-400" />
    ...
    const handleSaveClosing = async () => {
        ...
        // File Validation
        if (!closingProof) {
            alert('⚠️ Falta la Factura de Cierre de Caja (Siigo). Es obligatoria.')
            return
        }
        if (!productsProof) {
            alert('⚠️ Falta la Factura de Productos Vendidos (Siigo). Es obligatoria.')
            return
        }
        ...
        try {
            ...
            // 1. Upload Proofs
            const urlClosing = await uploadFile(closingProof, 'cierre_siigo')
            const urlProducts = await uploadFile(productsProof, 'productos_siigo')
            let urlCard = ''
            if (cardProof) {
                urlCard = await uploadFile(cardProof, 'datafono_siigo')
            }

            // Construct Notes with URLs
            let finalNotes = formData.notes
            finalNotes += `\n\n[SOPORTES ADJUNTOS]:`
            finalNotes += `\n📄 Cierre Siigo: ${urlClosing}`
            finalNotes += `\n📦 Productos Siigo: ${urlProducts}`
            if (urlCard) finalNotes += `\n💳 Datáfono: ${urlCard}`

            // 2. Insert Main Closing Record (SIIGO)
            // ASSUMING TABLE siigo_closings exists
            const { data: closingData, error: closingError } = await supabase.from('siigo_closings').insert({
            ...
            if (productsSold.length > 0) {
                ...
                const { error: prodError } = await supabase.from('siigo_closing_products').insert(productsToInsert)
                if (prodError) throw prodError
            }

            // 4. Insert Movements (Expenses + Tips) (SIIGO)
            ...
            if (movementsToInsert.length > 0) {
                const { error: movError } = await supabase.from('siigo_closing_movements').insert(movementsToInsert)
            ...
            <div className="flex items-center justify-between mb-8 px-4">
                {[1, 2, 3, 4, 5].map(i => (
                    <div key={i} className={`flex items-center ${i < 5 ? 'w-full' : ''}`}>
                        ...
                        {i < 5 && (
                            <div className={`h-1 w-full mx-2 rounded-full ${step > i ? 'bg-blue-600' : 'bg-gray-100'}`} />
                        )}
                    </div>
                ))}
            </div>
